...
uci_moves_list = [
    ...
    "h7h8r", "h7h8b", "a7a8n", "a7b8n", "b7a8n", "b7b8n", "b7c8n", "c7b8n",
    "c7c8n", "c7d8n", "d7c8n", "d7d8n", "d7e8n", "e7d8n", "e7e8n", "e7f8n",
    "f7e8n", "f7f8n", "f7g8n", "g7f8n", "g7g8n", "g7h8n", "h7g8n", "h7h8n"]



class TestFlatMapper(unittest.TestCase):

    def setUp(self):
        self.mapper = FlatMapper()
        print(f"Number of moves : {len(self.mapper.moves_list)}")

    def test_move_number(self):
        """Checks if the number of move is correct."""
        self.assertEqual(len(self.mapper.moves_list), 1858,
                         "Critical error : the move count is not equal to 1858")

    def test_round_trip(self):
        """Checks if index -> uci -> index is valid."""
        for idx in range(1858):
            uci = self.mapper.get_move_string(idx)
            recovered_idx = self.mapper.get_move_index(uci)
            self.assertEqual(idx, recovered_idx,
                             f"Inconsistency on index {idx} (UCI: {uci})")

    def test_standard_moves(self):
        """Vérifie quelques coups classiques."""
        # e2e4 (Début classique)
        for uci_move in uci_moves_list:
            idx = self.mapper.get_move_index(uci_move)
            self.assertIsNotNone(idx,f"{uci_move} should exist but does not.")

    def test_promotions_logic(self):

        idx_base = self.mapper.get_move_index("a7a8")
        idx_queen = self.mapper.get_move_index("a7a8q")

        self.assertIsNotNone(idx_base, "a7a8 doit exister")
        self.assertIsNotNone(idx_queen, "a7a8q doit être reconnu")
        self.assertEqual(idx_base, idx_queen, "a7a8 et a7a8q doivent pointer vers le même index")

        # 2. Sous-promotions (doivent avoir des index différents)
        idx_knight = self.mapper.get_move_index("a7a8n")
        idx_rook = self.mapper.get_move_index("a7a8r")

        self.assertNotEqual(idx_base, idx_knight, "La promotion Cavalier ne doit pas écraser la Dame")
        self.assertNotEqual(idx_knight, idx_rook, "Cavalier et Tour doivent être différents")

        # 3. Vérifier que c'est bien dans la liste
        self.assertIn("a7a8n", self.mapper.moves_list)
        self.assertIn("a7a8r", self.mapper.moves_list)
        self.assertIn("a7a8b", self.mapper.moves_list)

        # 4. Vérifier qu'une promotion impossible n'existe pas
        # Ex: Promotion au milieu du plateau (e4e5n) n'a aucun sens
        idx_fake = self.mapper.get_move_index("e4e5n")
        self.assertIsNone(idx_fake, "Une promotion au centre du plateau ne doit pas exister")

    def test_corner_cases(self):
        """Checks for corners (a1, h1, a8, h8)."""

        self.assertIsNotNone(self.mapper.get_move_index("a1b1"))

        self.assertIsNotNone(self.mapper.get_move_index("a1h8"))
